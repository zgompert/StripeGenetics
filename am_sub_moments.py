...
of = "dd_mom_"+p1+"_"+p2+"_am.txt"
pngf = "dd_mom_"+p1+"_"+p2+"_am.png"

ofile = open(of,"w")

ns =  fs_proj.sample_sizes

## define ancient migration (AM) model, includes exponential growth
def AMg(params, ns):
    ...
    ## period with ancient gene flow
    fs.integrate(nu_func, T, m=numpy.array([[0, m12], [m21, 0]]))
    ## gene flow comes to an end
    fs.integrate(nu_func, T2, m=numpy.array([[0, 0], [0, 0]]))
    return fs

## define ancient migration (AM) model, split into 2 pops 
def AM(params, ns):
    ...
    fs = moments.Manips.split_1D_to_2D(fs, ns[0], ns[1])
    ## period with ancient gene flow
    fs.integrate([nu1, nu2], T, m=numpy.array([[0, m12], [m21, 0]]))
    ## gene flow comes to an end
    fs.integrate([nu1, nu2], T2, m=numpy.array([[0, 0], [0, 0]]))
    return fs
