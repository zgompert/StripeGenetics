...
## read in data
gwa_p<-fread("output/o_ref_pattern_gs.assoc.txt",header=TRUE)
gwa_p_no8<-fread("output/o_ref_pattern_no8_gs.assoc.txt",header=TRUE)
gwa_c<-fread("output/o_ref_color_gs.assoc.txt",header=TRUE)
gwa_c_no8<-fread("output/o_ref_color_no8_gs.assoc.txt",header=TRUE)
...
cl<-1.45
pdf("gwa_refugio_gs.pdf",height=10,width=8)
par(mfrow=c(4,1))
